...
def calculate_accuracy(data,features,test,totalClass,foldSize):
    ...
    classSeparated = []
    #print np.size(np.array(test),0)

    test_X = np.transpose(test[:,:4])
    #print test_X.shape
    test_Y = np.reshape(test[:, 4],((foldSize),1))

    for i in range(np.size(test_X,1)):
        #compare class 1 and 2
        classifier1 = calculateClassifier(np.reshape(test_X[:,i],(4,1)),W1_cov_inv,W2_cov_inv,W1_cov,W2_cov,mean_class1,mean_class2)
        #print "classifier -1 ", classifier1
        if classifier1 > 0:
            #compare class 1 and 3
            classifier2 = calculateClassifier(np.reshape(test_X[:,i],(4,1)),W1_cov_inv,W3_cov_inv,W1_cov,W3_cov,mean_class1,mean_class3)
            ...
            classifier3 = calculateClassifier(np.reshape(test_X[:,i],(4,1)),W2_cov_inv,W3_cov_inv,W2_cov,W3_cov,mean_class2,mean_class3)
...
def calculateClassifier(X,covInv1,covInv2,cov1,cov2,mean1,mean2):
    ...
    if isnan(A) or isinf(A): A = 0.0
    if isnan(B) or isinf(B): B = 0.0  
     
    X2 = find_pseudodeterminant(cov2)
    variableLog2 = 0.0
    if X2>0:
        if isnan(np.log(X2)) or isinf(np.log(X2)) or isinstance(X2, complex) or isinstance(np.log(X2), complex): 
            variableLog2 = 0.0
        else: 
            variableLog2 = math.log(X2)
        
    X1 = find_pseudodeterminant(cov1)
    variableLog1 = 0.0
    if X1>0:
        if isnan(np.log(X1)) or isinf(np.log(X1)) or isinstance(X1, complex) or isinstance(np.log(X1), complex): 
            variableLog1 = 0.0
        else: 
            variableLog1 = math.log(X1)
